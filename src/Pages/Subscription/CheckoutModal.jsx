...
const getUserFromStorage = () => {
  try { return JSON.parse(localStorage.getItem("mlmuser") || "{}"); }
  catch { return {}; }
};
...
export function CheckoutModal({ plan, isOpen, setIsOpen, onBack, onPaymentSuccess }) {
  ...
  const handleConfirmPurchase = useCallback(async () => {
    ...
    try {
      ...
      if (!sdkLoaded) throw new Error("Payment gateway failed to load. Check your connection.");

      // 2. Create order on backend (amount in paise)
      const res = await fetch(`${CREATE_ORDER_URL}&amount=${payableAmount * 100}`);
      if (!res.ok) throw new Error(`Order creation failed: ${res.status}`);
      const orderData = await res.json();
      orderId = orderData?.data?.order_id ?? orderData?.order_id ?? orderData?.id;
      ...
      await new Promise((resolve, reject) => {
        ...
        rzp.on("payment.failed", async (failRes) => {
          ...
          const failDoc = { ...buildLogDoc("Failed"), OrderId: failedId, UTRID: failedId };
          await addDoc(collection(db, "paymentlog"), failDoc).catch(() => {});
          reject(new Error(failRes?.error?.description || "Payment failed. Please retry."));
        });
  ...
  const handleApplyCoupon = async () => {
    ...
    setPaymentError(null);
    try {
      const q = query(collection(db, "couponcode"), where("code", "==", couponString));
      const snap = await getDocs(q);
      if (snap.empty) {
        setCouponStatus("invalid"); setDiscountPercent(0); setCouponData(null);
        return;
      }
      const data = snap.docs[0].data();
      if (data.active === false) {
        setCouponStatus("inactive"); setDiscountPercent(0); setCouponData(null);
        return;
      }
      ...
      setCouponStatus("invalid"); setDiscountPercent(0);
    } finally {
            ...
            <Modal.Body className="space-y-4 pt-0">
              ...
              <div className="rounded-xl bg-black/5 dark:bg-white/5 border dark:border-white/10 border-black/10 px-4 py-3 flex items-center justify-between gap-2">
                <div className="text-center">
                  ...
                  <p className="text-xs font-semibold dark:text-white text-gray-800">{fmtDisplay(today)}</p>
                </div>
                ...
                <div className="text-center">
                  <p className="text-[10px] text-gray-400 uppercase tracking-wide mb-0.5">Expiry</p>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between px-1 min-h-[20px]">
                  <span className="text-xs font-medium">
                    {couponStatus === "valid" && (
                      <span className="text-green-500">✅ {discountPercent}% discount applied!</span>
                    )}
                    {couponStatus === "invalid" && (
                      <span className="text-red-500">❌ Coupon not found</span>
                    )}
                    {couponStatus === "inactive" && (
                      <span className="text-orange-500">⛔ Coupon is no longer active</span>
                    )}
                  </span>
              ...
              <div className="rounded-xl bg-black/5 dark:bg-white/5 border dark:border-white/10 border-black/10 p-4 space-y-2">
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500 dark:text-gray-400">Plan Amount</span>
                  <span className="dark:text-white text-gray-800 font-medium">₹{baseAmount}</span>
                </div>
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500 dark:text-gray-400">GST / Taxes</span>
                  <span className="dark:text-white text-gray-800 font-medium">Included</span>
                </div>
                <div className="h-px dark:bg-white/10 bg-black/10 my-1" />
                <div className="flex justify-between text-base font-bold">
                  <span className="dark:text-white text-gray-900">Total Payable</span>
              ...
              {paymentError && (
                <div className="rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-500/30 px-4 py-3">
                  <p className="text-xs text-red-600 dark:text-red-400 text-center">⚠️ {paymentError}</p>
                </div>
              )}
            </Modal.Body>
            ...
            <Modal.Footer className="flex flex-col gap-2 pt-2">
              ...
              >
                {paymentLoading ? (
                  ...
                  <><span>🔒</span> Confirm Purchase &nbsp;·&nbsp; ₹{finalAmount}</>
                )}
              </button>
