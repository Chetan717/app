...
// ── Aspect ratio lock (2:3 portrait) ─────────────────────────────
const ASPECT_RATIO = 2 / 3; // width / height  →  2:3 portrait

const INITIAL_W = 0.6;
const INITIAL_H = INITIAL_W / ASPECT_RATIO;   // 0.6 / (2/3) = 0.9
const INITIAL_CROP = {
  x: (1 - INITIAL_W) / 2,
  y: (1 - INITIAL_H) / 2,
  w: INITIAL_W,
  ...
  const onPointerMove = (e) => {
    ...
    setCrop(() => {
      ...
      if (ds.type === "move") {
        ...
        // ── Resize with locked 2:3 ratio ─────────────────────────
        // Drive width from horizontal drag → derive height = w / ratio
        // Each corner keeps its opposite edge fixed.
  ...
  return (
    <div className="flex dark:bg-black w-full flex-col items-center gap-4 p-3">
      ...
      <div className="flex flex-wrap justify-center gap-2">
        <button onClick={() => setRotation((r) => r - 90)} className="px-3 dark:text-black py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-sm font-medium transition">↺ Rotate</button>
        ...
        <button onClick={() => setCrop(INITIAL_CROP)} className="px-3 dark:text-black py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-sm font-medium transition">↺ Reset</button>
      </div>
